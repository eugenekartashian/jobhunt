...
At the start of any Codex session that involves UI work, read `ui-registry.md` before writing any component. When building a new card, check how existing cards were built. When building a new button, check what button patterns already exist. When building a new status badge, match the exact classes already in use.

The registry grows as the project grows. The more components are imprinted, the more consistent every new component becomes, because Codex has a precise reference for what already exists.
