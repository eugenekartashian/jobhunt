...
| -------------------- | ------------------------------------------ | -------------------------- |
| `landing_cta_clicked` | Homepage primary CTA clicked              | placement                  |
| `oauth_sign_in_started` | User clicks an OAuth provider           | provider                   |
| `$pageview`          | App Router page changes                    | $current_url               |
| `auth_signed_in`     | OAuth callback completes successfully      | userId, method             |
| `auth_signed_out`    | Server sign-out route runs before clearing cookies | userId            |
| `authenticated_placeholder_viewed` | Temporary protected page rendered | userId, page        |
