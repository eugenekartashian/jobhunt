...
| resume_pdf_key      | text        | InsForge Storage object key                  |
...
| is_tailored         | boolean     | True when tailored materials exist             |
| tailored_resume_url | text        | Optional tailored resume URL                   |
| tailored_resume_key | text        | Optional tailored resume storage key           |
| tailored_cover_letter | text     | Optional tailored cover letter content         |
| tailored_at         | timestamptz | Optional timestamp for tailored materials      |
...
New users are bootstrapped by `public.handle_new_user()` after `auth.users`
insert. The trigger creates a matching `profiles` row with auth `id`, `email`,
and best-effort OAuth display name.
