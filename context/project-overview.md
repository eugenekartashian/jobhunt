...
Job Hunt eliminates all of that preparation work. The agent finds the jobs, scores them intelligently against the user's actual skills, and researches each company so the user arrives at every application fully informed. The user just decides which ones to apply to and clicks.
...
auth_signed_in; // { userId, method }
auth_signed_out; // { userId }
authenticated_placeholder_viewed; // { userId, page }
